...
### Instructions ### 
A set of instructions that make the Agent follow a consistent, repeatable processing pattern. They are written as rules that formalize the issues and anti-patterns to **identify, explain, and fix**, so every object is assessed against the same criteria. Copy the content of the file below into the agent's **Instructions** box.
